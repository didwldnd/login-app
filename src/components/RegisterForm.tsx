...
function RegisterForm({ onClose }: RegisterFormProps) {
  ...
  const [validationAttempted, setValidationAttempted] = useState(false) // 유효성 검사 시도 여부 추적
  ...
  const handleRegister = async () => {
    setValidationAttempted(true) // 유효성 검사 시도 표시
    if (!validate()) return
  ...
  const isPasswordValid = () => {
    const status = getPasswordValidationStatus(form.password)
    return status.minLength && status.hasLetter && status.hasSpecialChar && status.noSpace
  }
    ...
    <form className="register-form">
      ...
      {fields.map((field) => (
        <div key={field} className="form-field">
          ...
          {/* 비밀번호 실시간 조건 검사 - 유효성 검사 시도 후 비밀번호가 유효하지 않을 때만 표시 */}
          {field === "password" &&
            validationAttempted &&
            !isPasswordValid() &&
            (() => {
              ...
              return (
                <div className="password-rules">
                  <p className={status.minLength ? "valid" : "invalid"}>• 최소 8자 이상</p>
                  <p className={status.hasLetter ? "valid" : "invalid"}>• 영문자 포함</p>
                  <p className={status.hasSpecialChar ? "valid" : "invalid"}>• 특수문자 포함 (@#$%^&+=!)</p>
                  <p className={status.noSpace ? "valid" : "invalid"}>• 공백 미포함</p>
                </div>
              )
            })()}
        </div>
